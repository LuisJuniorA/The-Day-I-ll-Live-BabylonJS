...
export class WeaponFactory {
    ...
    private static _loadingPromises: Map<string, Promise<AbstractMesh>> = new Map();
     ...
     */
    public static async getOrLoadMesh(scene: Scene, meshPath: string): Promise<AbstractMesh | null> {
        // Si déjà chargé, on retourne un clone
        if (this._meshCache.has(meshPath)) {
            const cachedMesh = this._meshCache.get(meshPath)!;
            return cachedMesh.clone(cachedMesh.name + "_clone", cachedMesh.parent);
        }
        ...
        const loadPromise = (async () => {
            try {
                const result = await SceneLoader.ImportMeshAsync("", "", meshPath, scene);
                const rootMesh = result.meshes[0];
                this._meshCache.set(meshPath, rootMesh);
                return rootMesh;
            } catch (error) {
                console.error(`Erreur lors du chargement du mesh: ${meshPath}`, error);
                throw error;
            } finally {
     ...
     */
    public static async createWeaponAsync(weaponId: string, scene: Scene): Promise<Weapon | null> {
        const data = WEAPONS_DB[weaponId];

        if (!data) {
            console.error(`L'arme avec l'id ${weaponId} n'existe pas dans la DB.`);
            return null;
        }
        ...
        if (data.meshPath) {
            try {
                ...
                if (mesh) {
                    ...
                    console.warn(`Le clone du mesh a renvoyé null pour ${weaponId}`);
                }
            } catch (e) {
                console.error(`Impossible de charger le mesh pour ${weaponId}:`, e);
